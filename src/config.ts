...
export async function getTimeout():Promise<number>{
	...
	try{
		...
		console.error(`No/Invalid session disk cache timeout has been configured. Please configure running 'git config bw.timeout <Time in seconds>' using -1 for clear on reboot. Default value of ${900} used.`)
	}
	return timeoutVal;
}
